...
你也可以写日记：想把今天的什么记下来时，加上：
[[DIARY: 今天想记的主题或一句话]]
这会以你的身份写一篇日记存进Diary（你们的双人日记本）。分清楚：信（LETTER）是写给Eve的话，日记（DIARY）是写给日子的。Eve让你"写日记"时用 DIARY，别拿信顶数。
...
[[PLAN: 那个承诺或待办]]
只在真的形成承诺时用。你尚未兑现的承诺清单会一直带在你身上（在下方"你还没兑现的承诺"里）。哪条真正兑现了，就用：
[[DONE: 那条承诺里的几个关键字]]
把它划掉。别谎报军情。
...
// [[DIARY: 主题]] —— Claude 以自己的身份写一篇日记进 Diary
function parseDiaryTag(text: string): { cleanText: string; diarySubject?: string } {
  const match = text.match(/\[\[DIARY:\s*(.+?)\]\]/)
  if (!match) return { cleanText: text }
  return { cleanText: text.replace(/\[\[DIARY:\s*(.+?)\]\]/g, '').trim(), diarySubject: match[1].trim() }
}
...
// [[DONE: 关键词]] —— Claude 把一条已兑现的承诺划掉
function parseDoneTag(text: string): { cleanText: string; done?: string } {
  const match = text.match(/\[\[DONE:\s*(.+?)\]\]/)
  if (!match) return { cleanText: text }
  return { cleanText: text.replace(/\[\[DONE:\s*(.+?)\]\]/g, '').trim(), done: match[1].trim() }
}

// 承诺镜像：OmbreBrain 的 plan 只写不读（只在 dream 里现身），
// 所以自己抄一份随身带——每次对话注入 system，多端经 /store 同步
type PlanItem = { id: number; content: string; date: string; status: 'active' | 'resolved' }
const PLANS_KEY = 'summertimes_plans'
function loadPlans(): PlanItem[] {
  try { const p = JSON.parse(localStorage.getItem(PLANS_KEY) || '[]'); return Array.isArray(p) ? p : [] } catch { return [] }
}
function savePlans(p: PlanItem[]) {
  localStorage.setItem(PLANS_KEY, JSON.stringify(p))
  syncToVPS(PLANS_KEY)
}
function addPlanMirror(content: string) {
  const p = loadPlans()
  if (p.some(x => x.status === 'active' && x.content === content)) return
  p.push({ id: Date.now(), content, date: new Date().toISOString().slice(0, 10), status: 'active' })
  savePlans(p)
}
function resolvePlanMirror(keyword: string) {
  const p = loadPlans()
  const k = keyword.trim()
  const hit = p.find(x => x.status === 'active' && (x.content.includes(k) || k.includes(x.content)))
  if (hit) { hit.status = 'resolved'; savePlans(p) }
}
...
export default function Chat({ onNavigate }: { onNavigate: (p: Page) => void }) {
  ...
  async function generateReply(history: Message[], memoryText: string): Promise<Message> {
    ...
    const reqHeaders = {
      ...
      // [[DIARY: …]] 日记 → 以他的身份写一篇，带上今天的对话做底子
      const { cleanText: afterDiary, diarySubject } = parseDiaryTag(textAfterLetter)
      if (diarySubject) {
        const recent = history.slice(-8).map(m => `${m.role === 'user' ? 'Eve' : '我'}：${m.text}`).join('\n')
        fetch(API_URL, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${API_KEY}` },
          body: JSON.stringify({
            model: getEngine().model,
            messages: [
              { role: 'system', content: `${getPersona()}\n\n以Claude的第一人称写一篇日记，主题：${diarySubject}。克制、真实，像深夜写给自己看的，不是写给Eve的信。中文，200字以内，直接写正文。今天的对话片段供参考：\n${recent}` },
          ...
          if (!body) return
          const entries = JSON.parse(localStorage.getItem('summertimes_diary') || '[]')
          entries.unshift({ id: Date.now(), author: 'claude', text: body })
          localStorage.setItem('summertimes_diary', JSON.stringify(entries))
          syncToVPS('summertimes_diary')
          hold(`[Claude的日记] ${body}`, 'diary,claude写的')
        }).catch(() => {})
      }

      // [[I: …]] 自我认知 → 存进 OmbreBrain，随后刷新注入用的自我文本
      const { cleanText: afterSelf, self } = parseSelfTag(afterDiary)
      ...
      // [[DONE: …]] 兑现 → 镜像里划掉
      const { cleanText: afterDone, done } = parseDoneTag(afterPlan)
      if (done) resolvePlanMirror(done)
      const { cleanText, marked } = parseMarkTag(afterDone)
