            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 10 }}>
                ...
                <input type="date" value={deadline} onChange={e => setDeadline(e.target.value)}
                  style={{ marginLeft: 'auto', background: 'rgba(var(--ink),0.08)', border: '0.5px solid rgba(var(--ink),0.15)', borderRadius: 8, padding: '4px 8px', fontFamily: "'Cormorant Garamond', serif", fontSize: 13, color: 'rgba(var(--ink),0.7)', outline: 'none', colorScheme: 'dark' }} />
              </div>
              <button onClick={add} style={{ marginTop: 12, width: '100%', background: 'rgba(var(--ink),0.15)', border: '0.5px solid rgba(var(--ink),0.2)', borderRadius: 10, padding: '8px', fontFamily: "'Cormorant Garamond', serif", fontSize: 14, color: 'rgba(var(--ink),0.88)', cursor: 'pointer', letterSpacing: 2 }}>add</button>
            </motion.div>
          )}
        </AnimatePresence>
            ...
            {sorted.map(r => (
              <motion.div key={r.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, x: -20 }}
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10 }}>
                  ...
                  <p style={{ flex: 1, fontSize: 15, lineHeight: 1.65, color: 'rgba(var(--ink),0.88)', textDecoration: r.done ? 'line-through' : 'none', fontFamily: "'Cormorant Garamond', serif" }}>{r.text}</p>
                  <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                    <button onClick={() => pin(r.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, opacity: r.pinned ? 1 : 0.4, color: 'rgba(var(--ink),0.8)' }}>◈</button>
                    <button onClick={() => toggle(r.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, opacity: r.done ? 1 : 0.4, color: 'rgba(var(--ink),0.8)' }}>✓</button>
                    <button onClick={() => remove(r.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 14, opacity: 0.4, color: 'rgba(var(--ink),0.8)' }}>×</button>
                  </div>
                </div>
                {r.deadline && <p style={{ fontSize: 11, color: 'rgba(var(--ink),0.4)', marginTop: 6, marginLeft: 18, letterSpacing: 1, fontStyle: 'italic' }}>{r.deadline}</p>}
                {r.pinned && <p style={{ fontSize: 10, color: 'rgba(var(--ink),0.35)', marginTop: 4, marginLeft: 18, letterSpacing: 2 }}>pinned</p>}
              </motion.div>
            ))}
          </AnimatePresence>
